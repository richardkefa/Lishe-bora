...
from .. import db
from ..requests import get_local_time, query_api
...
def comment(blog_id):
    ...
    new_comment.save()
    return redirect(url_for('main.blog',id = blog.id))
@main.route('/comment/<int:blog_id>/delete',methods = ['POST'])
def delete_comment(blog_id):
    blog = Blog.query.get_or_404(blog_id)  
    comment = Comment.query.filter_by(blog_id = blog.id).first()
    if blog.user != current_user:
        abort(403)                                                                                          
...
def subscribe():
    ...
    mail_message("Subscribed to ","email/welcome_subscriber",new_subscriber.email,new_subscriber=new_subscriber)
